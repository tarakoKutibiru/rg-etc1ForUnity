...
namespace rg_etc1
{
   ...
   template <typename T>
   inline void zero_object(T &obj) { memset((void *)&obj, 0, sizeof(obj)); }
   template <typename T>
   inline void zero_this(T *pObj) { memset((void *)pObj, 0, sizeof(*pObj)); }
...
#define RG_ETC1_ARRAY_SIZE(X) (sizeof(X) / sizeof(decay_array_to_subtype(X)))
   ...
   static const int g_etc1_inten_tables[cETC1IntenModifierValues][cETC1SelectorValues] =
       {
           {-8, -2, 2, 8}, {-17, -5, 5, 17}, {-29, -9, 9, 29}, {-42, -13, 13, 42}, {-60, -18, 18, 60}, {-80, -24, 24, 80}, {-106, -33, 33, 106}, {-183, -47, 47, 183}};
...
#undef RG_ETC1_GET_KEY_FROM_INDEX
   ...
   {
      inline etc1_solution_coordinates() : m_unscaled_color(0, 0, 0, 0),
                                           m_inten_table(0),
                                           m_color4(false)
      {
      }

      inline etc1_solution_coordinates(uint r, uint g, uint b, uint inten_table, bool color4) : m_unscaled_color(r, g, b, 255),
                                                                                                m_inten_table(inten_table),
                                                                                                m_color4(color4)
      {
      }

      inline etc1_solution_coordinates(const color_quad_u8 &c, uint inten_table, bool color4) : m_unscaled_color(c),
                                                                                                m_inten_table(inten_table),
                                                                                                m_color4(color4)
      {
      }
   ...
   public:
      ...
      struct params : etc1_pack_params
      {
         ...
         params(const etc1_pack_params &base_params) : etc1_pack_params(base_params)
         {
   ...
   bool etc1_optimizer::evaluate_solution_fast(const etc1_solution_coordinates &coords, potential_solution &trial_solution, potential_solution *pBest_solution)
   {
      ...
      for (int inten_table = cETC1IntenModifierValues - 1; inten_table >= 0; --inten_table)
      {
         ...
         if ((m_pSorted_luma[n - 1] * 2) < block_inten_midpoints[0])
         {
            ...
            {
               const uint min_error = static_cast<uint>(labs(static_cast<int>(block_inten[0]) - static_cast<int>(m_pSorted_luma[n - 1])));
         ...
         else if ((m_pSorted_luma[0] * 2) >= block_inten_midpoints[2])
         {
            ...
            {
               const uint min_error = static_cast<uint>(labs(static_cast<int>(m_pSorted_luma[0]) - static_cast<int>(block_inten[3])));
               if (min_error >= trial_solution.m_error)
                  continue;
            }
